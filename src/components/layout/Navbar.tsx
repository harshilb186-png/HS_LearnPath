    ...
    <header className="sticky top-0 z-50 w-full border-b border-white/[0.05] bg-background/60 backdrop-blur-3xl">
      <div className="container mx-auto px-6 h-20 flex items-center justify-between">
        <Link href="/" className="flex items-center gap-4 group transition-transform hover:scale-105">
          ...
          <span className="font-headline font-bold text-2xl tracking-tighter hidden sm:inline-block text-foreground dark:text-white uppercase italic">
            HS LearnPath<span className="text-primary">+</span>
          </span>
        </Link>

        <nav className="hidden md:flex items-center bg-secondary/40 dark:bg-secondary/40 p-1.5 rounded-full border border-white/5 dark:border-white/5 backdrop-blur-md">
          {NAV_ITEMS.map((item) => {
              ...
              <DropdownMenuContent className="w-64 glass-card mt-3 rounded-2xl p-2" align="end" forceMount>
                <DropdownMenuLabel className="font-normal p-4">
                  <div className="flex flex-col space-y-1">
                    <p className="text-sm font-bold text-foreground dark:text-white leading-none tracking-tight">{user.displayName || user.email?.split('@')[0]}</p>
                ...
                <DropdownMenuItem asChild className="focus:bg-primary/10 p-3 rounded-xl cursor-pointer">
                  <Link href="/profile" className="flex w-full items-center">
                    ...
                    <span className="font-bold text-[10px] uppercase tracking-widest text-foreground dark:text-white">Profile</span>
                  </Link>
                </DropdownMenuItem>
                <DropdownMenuItem asChild className="focus:bg-primary/10 p-3 rounded-xl cursor-pointer">
                  <Link href="/settings" className="flex w-full items-center">
                    <Settings className="mr-3 h-4 w-4 text-accent" />
                    <span className="font-bold text-[10px] uppercase tracking-widest text-foreground dark:text-white">Settings</span>
                  </Link>
                </DropdownMenuItem>
                <DropdownMenuSeparator className="bg-white/5 mx-2" />
